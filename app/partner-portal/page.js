...
export default function PartnerPortal() {
  ...
  // Reset page when coupon changes
  useEffect(() => {
    setCurrentPage(1);
  }, [debouncedCoupon]);
  ...
  useEffect(() => {
    ...
    const fetchOrders = async () => {
      ...
      try {
        const res = await fetch(`/api/partner-portal?coupon=${encodeURIComponent(debouncedCoupon)}&page=${currentPage}&limit=5`);
